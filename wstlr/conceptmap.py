    ...
    with open(harmony_csv, "rt") as f:
        ...
        for line in reader:
            if line["table_name"].strip() != "":
                ...
                if (
                    target_code
                    in mappings[local_cs]["group"][target_cs]["codes"][local_code][
                        "target_codes"
                    ]
                ):
                    redundant_notice[f"{local_cs}:{local_code}"][
                        f"{target_cs}:{target_code}"
                    ] += 1
                ...
                vs_sources[vss_key].append(
                    {"code": local_code, "display": line["text"]}
                )
    ...
    for vs_key in vs_sources:
        ...
        for coding in vs_sources[vs_key]:
            ...
            cm_obj["source_codes"][-1]["codes"].append(
                {"code": code, "display": coding["display"]}
            )
    ...
    for local_cs in mappings:
        ...
        for target_cs in mappings[local_cs]["group"]:
            ...
            for source_code in mappings[local_cs]["group"][target_cs]["codes"]:
                source_mapping = mappings[local_cs]["group"][target_cs]["codes"][
                    source_code
                ]
                element = {
                    "code": source_code,
...
class ConceptMap:
    ...
    def prepare_for_whistle(
        self, source_id, source_name, source_title, target_id, target_name, target_title
    ):
        ...
        for source_system in self.mappings:
            for target_system in self.mappings[source_system]:
                cm["group"].append(
                    {"source": source_system, "target": target_system, "element": []}
                )
                for source_code in self.mappings[source_system][target_system]:
                    ...
                    for target_code in self.mappings[source_system][target_system][
                        source_code
                    ]:
    ...
    def add_mapping(self, source, target):
        ...
        self.mappings[source["system"]][target["system"]][source["code"]].add(
            target["code"]
        )
    ...
    def generate_valueset(self, codings, id, name, title, curies):
        ...
        for system in codings:
            ...
            if system in curies:
                curie = curies[system] + ":"
            vs["compose"]["include"].append({"system": system, "concept": []})
...
def BuildConceptMap(
    csvfilenames, curies, name_prefix=None, outname=None, codesystems=[]
):
    # We'll assume that we only want to filename with any path/dot information

    if name_prefix is None:
        name_prefix = csvfilenames[0].split("/")[-1].split(".")[0]

    if outname is None:
        outname = ".".join(csvfilenames[0].split(".")[0:-1]) + ".json"
    ...
    if True:
            ...
            with open(csvfilename, "rt") as f:
                ...
                for row in reader:
                    ...
                    if key not in observed_mappings:
                        ...
                        mappings[row["local code system"]][row["code system"]].append(
                            row
                        )
        ...
        for source in mappings.keys():
            ...
            for target in mappings[source].keys():
                ...
                element = {"source": source, "target": the_target, "element": []}
