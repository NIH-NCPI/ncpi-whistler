...
def identifier_columns(key_columns, subject_id, varpath="row"):
    if key_columns is None and subject_id is None:
        sys.stderr.write(
            "You must have a subject_id column or key columns in the configuration. Please see docs that do not currently exist!\n"
        )
        sys.exit(1)
    ...
    def __init__(self, ddtable, ddconfig, id_colname):
        ...
        if "key_columns" in ddconfig:
            self.colnames = identifier_columns(
                ddconfig["key_columns"], id_colname, "row"
            )  # [fix_fieldname(x) for x in ddconfig['key_columns'].split(",")]
        ...
        if self.table_type == TableType.Embedded:
            ...
            )  # [fix_fieldname(x) for x in ddconfig['group_by'].split(",")]
