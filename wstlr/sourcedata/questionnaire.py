...
def exec():
    ...
    for config_file in args.config:
        ...
        with open(template_dir / "questionnaires.wstl", 'rt') as template_file:
            ...
            with open(whistle_src_dir / "source_data_questionnaires.wstl", 'wt') as outf:
                print(outf.name)
                outf.write(template.render(tables=dd_table_data, 
                                            TableType=TableType, 
                                            use_profiles = args.no_profiles == False))

        print("Please add the function, BuildRawDataQR to your main transform library function")        
